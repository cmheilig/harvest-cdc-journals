...
pd.DataFrame([process_aTag(aTag, home_dframe.url[0]) 
    ...
    to_excel('mmwr-home-anchors.xlsx', engine='openpyxl')
...
pd.DataFrame([process_aTag(aTag, url) 
    ...
    to_excel('mmwr-series-anchors.xlsx', engine='openpyxl')
...
# [44, 35, 38, 21] # 138
# reorganize 4 nested lists as a sinlge list of 134
...
#                               path                       string
# 0             /mmwr/index2021.html             Volume 70 (2021)
# 1             /mmwr/index2020.html             Volume 69 (2020)
# 2             /mmwr/index2019.html             Volume 68 (2019)
# 3             /mmwr/index2018.html             Volume 67 (2018)
# 4             /mmwr/index2017.html             Volume 66 (2017)
# ..                             ...                          ...
# 133  /mmwr/preview/ind1985_su.html             Volume 34 (1985)
# 134           /mmwr/index2022.html                Weekly Report
# 135          /mmwr/indrr_2021.html  Recommendations and Reports
# 136          /mmwr/indss_2022.html       Surveillance Summaries
# 137          /mmwr/ind2022_su.html                  Supplements

# Check for duplicate values
# volumes_dframe.path.drop_duplicates() # drops from 138 to 126
...
# [ 40,  41,  42,  43,  75,  76,  77,  78, 
#  113, 114, 115, 116, 134, 135, 136, 137]
volumes_dframe.loc[volumes_repeated, ['path', 'string']] 
# on inspection, keep indices 40, 41, 42, 43 - vol type corresponds to ser type
# delete 12 indices: 75,  76,  77,  78, 113, 114, 115, 116, 134, 135, 136, 137
volumes_dframe = volumes_dframe.drop(\
   [75,  76,  77,  78, 113, 114, 115, 116, 134, 135, 136, 137])
# Check again for duplicate values
# volumes_dframe.loc[volumes_dframe.path.duplicated(keep = False)].index # []
volumes_dframe.index = list(range(126))

volumes_html = [get_html_from_url(url) for url in tqdm(volumes_dframe.url)] # list of 126
# 126/126 [00:28<00:00,  4.46it/s]
# [len(x) for x in volumes_html]
# [294414, 300349, 273408, 283545, 303795, 389800, 385758, 134350, 117893, ...]
volumes_soup = [BeautifulSoup(html, 'lxml') for html in tqdm(volumes_html)]
# 126/126 [00:03<00:00, 36.31it/s]

# review all anchor-hrefs from volumes URLs
# [len(soup.find_all('a', href=True)) for soup in volumes_soup]
# [598, 645, 517, 581, 728, 743, 688, 722, 627, 592, ...] # len 126, sum 31141
# pd.DataFrame([process_aTag(aTag, url) 
#     for soup, url in zip(volumes_soup, volumes_dframe.url) 
#     for aTag in soup.find_all('a', href=True)]).\
#     to_excel('mmwr-volumes-anchors.xlsx', engine='openpyxl')
# [29010 rows x 7 columns]
...
# sum(articles_a_n) # 14630
# reorganize 126 nested lists as a single list of 14630

articles_dframe = pd.DataFrame([process_aTag(aTag, url) 
   for a_list, url in zip(articles_a, volumes_dframe.url)
   for aTag in a_list])
# articles_dframe.shape # (14630, 7)
# with pd.option_context("display.max_colwidth", 36):
#    display(articles_dframe.loc[:, ['path', 'string']])
#                                       path                               string
# 0       /mmwr/volumes/70/wr/mm705152a1.htm  COVID-19 Vaccine Safety in Child...
# 1       /mmwr/volumes/70/wr/mm705152a2.htm  Interim Estimate of Vaccine Effe...
# 2       /mmwr/volumes/70/wr/mm705152a3.htm  Characteristics and Clinical Out...
# 3       /mmwr/volumes/70/wr/mm705152e1.htm  Evaluation of a Test to Stay Str...
# 4       /mmwr/volumes/70/wr/mm705152e2.htm  Evaluation of Test to Stay Strat...
#                                    ...                                  ...
# 14625  /mmwr/preview/mmwrhtml/00026330.htm  Guidelines for the Prevention an...
# 14626  /mmwr/preview/mmwrhtml/00014715.htm  Human Immunodeficiency Virus Inf...
# 14627  /mmwr/preview/mmwrhtml/00023587.htm  Recommendations for Prevention o...
# 14628  /mmwr/preview/mmwrhtml/00001773.htm  Premature Mortality in the Unite...
# 14629  /mmwr/preview/mmwrhtml/00001712.htm  Summaries of Current Intelligenc...

# articles_dframe.to_excel("articles_dframe.xlsx", engine='openpyxl')
...
# {'href': 16, 'url': 18, 'path': 18, 'filename': 18, 'string': 1884}
...
# [ 4470,  4513,  7235,  7236,  8013,  8020,  8029,  8036,  8387,
#   8413,  8743,  8744, 13575, 13577, 14290, 14293, 14508, 14509]
# on inspection, keep rows 4470, 7235, 8013, 8387, 8743, 13575, 14290, 14508
# drop 10 rows: 4513, 7236, 8020, 8029, 8036, 8413, 8744, 13577, 14293, 14509
articles_dframe = articles_dframe.drop(\
   [4513, 7236, 8020, 8029, 8036, 8413, 8744, 13577, 14293, 14509])
articles_dframe.index = list(range(14620))
...
mmwr_dframe = pd.concat([
   ...
   axis = 0, ignore_index = True) # mmwr_dframe.index = list(range(13800))
# (14226, 7)
...
mmwr_dframe.to_excel('mmwr_dframe.xlsx', engine='openpyxl')
# Excelternatives
...
# articles_pdf_dframe.to_excel("articles_pdf_dframe.xlsx", engine='openpyxl')
...
mmwr_pdf_dframe.to_excel("mmwr_pdf_dframe.xlsx", engine='openpyxl')
