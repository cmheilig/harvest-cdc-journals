...
2. Convert to **Unicode (UTF-8) HTML**, cleaning up anomalies. [2_html.py](pycode/2_html.py)
   - Refine character classes to normalize **newlines and other spaces** and remove extras. [2_html_reduce-space.py](pycode/2_html_reduce-space.py)
   - Figure out how much space can be saved by **reducing `<svg>` elements**. [2_html_reduce-svg.py](pycode/2_html_reduce-svg.py)
...
4. Extract and organize **other metadata** as available, including digital obeject identifier, title, keywords, description, and author(s). [4_metadata.py](pycode/4_metadata.py)
   - Investigate **elements in `<head>`**, especially `<meta>` elements with `name` or `property` attributes. [4_metadata_detail.py](pycode/4_metadata_detail.py)
